def quick_sort(nums, start, end):
    if start >= end:
        return
    pivot = nums[start]  # arr의 첫 번째 요소를 pivot으로 지정
    low, high = start + 1, end  # low, high 지정

    # 모든 요소 순회: 각 요소를 pivot과 비교
    while low <= high:
        while low <= high and nums[low] <= pivot: low += 1  # pivot 보다 큰 값 찾기
        while low <= high and nums[high] >= pivot: high -= 1  # pivot 보다 작은 값 찾기
